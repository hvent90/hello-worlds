...
float noise_crater(float x, float z) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            ...
            float radius = 0.3f + 0.2f * (h * 0.5f + 0.5f); // Radius between 0.3 and 0.5
    ...
    float d = min_dist / crater_radius; // Normalized distance to center (0=center, 1=edge)
    
    if (d > 1.0f) return 0.0f; // Outside crater
...
float evaluate_moon_noise(float x, float z) {
    ...
    float maria_height = -0.5f + noise_fbm(x * 2.0f, z * 2.0f, 2, 2.0f, 0.5f) * 0.1f;
